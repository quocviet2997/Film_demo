...
<body onload="loadCategory()">
  ...
  <form action="<c:url value='/admin-api-category'/>" id="formSubmit" method="get">
  <div class="myContainer">
		<h1 class="text-center">List of Category</h3>
		<hr>
		<div class="text-left">
			<a href="<c:url value='/admin-api-category/new' />" class="btn btn-success">Add New Category</a>
		</div>
		<br>
					...
					$.each(result.data, (index, row) => {
						...
						= `<tr>
							...
							<td><div class="myMargin">
									<input type="button" class="btn btn-secondary btn-block" onclick="location.href='/admin-api-category/`+ row.id+`';" value="Edit" />
									<input type="button" class="btn btn-secondary btn-block" onclick="deleteCategory(`+ row.id+`);" value="Delete" />
								</div>
							</td>
						   </tr>`;
						$('#categoryTable tbody').append(rowContent);
					});
